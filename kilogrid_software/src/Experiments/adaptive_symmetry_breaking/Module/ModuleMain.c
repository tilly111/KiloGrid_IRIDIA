...
void send_message_to_modules(){
    ...
    msg->id = current_colour[1];//get_random(30000, 60000);  // dont know if id is important - maybe to check if msg arrived twice or so - max 65,535
    msg->data[1] = 5; // x sender 
    msg->data[2] = 5; // y sender 
    msg->data[3] = 20 ; // range
    msg->data[4] = current_colour[1]; // information
    msg->data[5] = some_cycle_counter - last_cycle_counter; // debug info
    msg->data[6] = 0;
    msg->data[7] = 0;
...
void CAN_rx(CAN_message_t *m){
    ...
    else if (m->data[0] == CAN_MODULE_TO_MODULE){  // read msg
        for(uint8_t cell_can_it = 0; cell_can_it < 4; cell_can_it++){
            if (sqrt(pow(fabs(m->data[1] - cell_x[cell_can_it]), 2) + pow(fabs(m->data[2] - cell_y[cell_can_it]), 2)) < m->data[3]) {
                for(uint8_t tmp_it = 0; tmp_it < 8; tmp_it++){
                    tmp_can_data[tmp_it] = m->data[tmp_it];
